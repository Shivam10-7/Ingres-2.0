...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!state && !district && !block) {
      return res.status(400).json({ error: 'At least one of state, district, or block must be provided' });
    }
    ...
    if (years !== undefined) {
      ...
      if (!years.length) years = [latestYear];
      const invalidYear = years.find(y => !YEAR_TABLE_MAP[y]);
      if (invalidYear) {
        const validYears = Object.keys(YEAR_TABLE_MAP).join(', ');
        return res.status(400).json({ error: `Unsupported year ${invalidYear}. Valid years: ${validYears}` });
      }
    }

    const data = await aggregateByYear({ state, district, block, years });
    res.json({ locationSummary: { state, district, block }, years: data });
  } catch (error) {
    next(error);
  }
});
