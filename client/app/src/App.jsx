...
import { 
  Plus, Search, MessageSquare, Folder, ChevronDown, Menu,
  Sparkles, Zap, Search as SearchIcon, BarChart3, 
  Send, MoreHorizontal, Database, X, ChevronRight,
  User, Clock, Shield, Code, Building2, TrendingUp,
  Mic
} from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="flex h-screen w-full bg-gradient-radial overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full relative">
        {/* Header - Simplified */}
        <header className="h-16 glass-panel border-b-0 flex items-center px-6 shrink-0">
          {!sidebarOpen && (
            <button
              onClick={() => setSidebarOpen(true)}
          ...
          <h1 className="text-lg font-semibold text-white">INGRES ChatBOT</h1>
        </header>
        ...
        <div className="flex-1 flex overflow-hidden">
                ...
                <div className="glass-card rounded-2xl p-2 flex items-center gap-2">
                  ...
                  <div className="relative" ref={modeDropdownRef}>
                    <button 
                      ...
                      className="p-2.5 rounded-xl hover:bg-white/10 transition-colors flex items-center gap-1"
                    >
                      <Plus className="w-5 h-5 text-white/60" />
                      <ChevronDown className={`w-3 h-3 text-white/40 transition-transform ${showModeDropdown ? 'rotate-180' : ''}`} />
                    </button>
                  ...
                  <div className="flex items-center gap-2 px-2 py-1 rounded-lg bg-white/5">
                    <selectedMode.icon className="w-4 h-4 text-blue-400" />
                    <span className="text-xs text-white/60">{selectedMode.label}</span>
                  </div>
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-white placeholder:text-white/40 text-sm py-3 px-2 focus:outline-none"
                  />
          ...
          {showDataPanel && (
            <div className="w-96 glass-panel border-l border-white/5 flex flex-col animate-slideIn">
              <div className="p-4 border-b border-white/5">
                <h3 className="text-sm font-semibold text-white/80 uppercase tracking-wider">Data Query</h3>
              </div>
                      ...
                      <table className="data-table text-sm">
                        ...
                        <tbody>
                          {SAMPLE_DATA.map((row, index) => (
                            <tr key={index}>
                              <td className="text-white/80">{row.year}</td>
                              <td className="text-white/60">{row.extractable}</td>
                              <td className="text-white/60">{row.extraction}</td>
                              <td className="text-white/60">{row.stage}</td>
                              <td>
